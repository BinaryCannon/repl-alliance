...
class ReplBase extends EventEmitter {
    ...
    addActions() {
        this.actions.help = new Command('help', [this.type.command],
            function (action) {
                console.log(this.mode().help(action));
            },
            'Finds out the help information for an action.');
        this.actions.use = new Command('use', [this.type.mode], function (mode) {
                this.setMode(mode);
            }, "Change mode.")
    }
