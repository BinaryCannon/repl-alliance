...
const {rest, string, boolean, file} = Type;
...
class ReplAlliance extends ReplBase {
    constructor(dirname = '.', environment = 'development') {
        super();
        this.dir = path.resolve(dirname);
    ...
    start(config, script) {
        this.network = require(path.join(this.dir, 'truffle-config.js')).networks[this.environment];

        this.eth = new Eth(new Eth.HttpProvider('http://' + this.network.host + ':' + this.network.port), {});

        return this.eth.net_version().then((netId) => {
            this.addContracts(this.dir, netId);
    ...
    addContracts(dir, netId) {
        const contractsDir = path.join(dir, 'build', 'contracts');
